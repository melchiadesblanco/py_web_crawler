...
class FormreqSpider(Spider):
    ...
    def parse(self, response):
        ...
        yield FormRequest('https://www.hkexnews.hk/sdw/search/searchsdw.aspx', formdata=data, callback=self.parse_form_page)
 
    def parse_form_page(self, response):
        #At this point, the result should be visible in response?
        #Or should I use fetch to parse the page??? How?
        #PARSING PHASE
        ...
        for tr in trs:
            ...
            number_issues = tr.xpath('.//td[@class="col-shareholding-percent text-right"]/div[@class="mobile-list-body"]/text()').extract_first()
